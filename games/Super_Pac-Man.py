...
def main():
    ...
    countNum = 0
    with open("data/save_file_SP.txt", "r") as f:
        for line in f:
            LINES += 1
            for s in line.split("     "):
                countNum += 1
    ...
    levels = read("data/levels_SP.txt")  # read the level from the text file
...
def runLevel(levels, num):
    ...
    while True:
        ...
        fruit = gameDict["fruits"]  # array of coordinates of pac-dots
        keys  = gameDict["keys"]
        ...
        if not levelIsFinished:
            ...
            if playerx + addX < len(mazeArray):
                if mazeArray[playerx + addX][playery + addY] not in ("#"):
                    if ((playerx + addX, playery + addY) not in lines and (playerx + addX, playery + addY) not in linesV) or superMode == True:  # if player did not collide with wall
                        if (playerx + addX, playery + addY) in fruit:  # if player made contact with pac-dot
                        ...
                        if (playerx + addX, playery + addY) in gameDict["superPellet"]:
                            superMode = True
                            PACMAN = {"right": pygame.image.load("assets/pacmanR-S.png"),
                                      "left": pygame.image.load("assets/pacmanL-S.png"),
                                      "up": pygame.image.load("assets/pacmanU-S.png"),
                                      "down": pygame.image.load("assets/pacmanD-S.png"),
                                      "rightC": pygame.image.load("assets/pacmanR_C-S.png"),
                                      "leftC": pygame.image.load("assets/pacmanL_C-S.png"),
                                      "upC": pygame.image.load("assets/pacmanU_C-S.png"),
                                      "downC": pygame.image.load("assets/pacmanD_C-S.png")}
                            superTime = time.time()
            ...
            if gameDict["fruitCounter"] == gameDict["total_fruits"]:  # level is finished if all pac-dots are eaten
        ...
        for i in range(LIVES):
            imageRect = pygame.image.load("assets/pacmanL.png").get_rect()
            imageRect.center = (x, 680)
            SCREEN.blit(pygame.image.load("assets/pacmanL.png"), imageRect)
            x += 30
        if LIVES <= 5:
            ...
            if num == len(levels) - 1:  # if last level
                ...
                if SCORE > HIGH_SCORE_MIN or LINES < 5:
                    if askName == False:  # askName: boolean value used to make sure the block of code is only run once
                        ...
                        save_file = open("data/save_file_SP.txt", "r")
                        lines = save_file.readlines()
                        save_file.close()

                        save_file = open("data/save_file_SP.txt", "w")
...
def start():
    ...
    # Title image
    title = pygame.image.load("assets/title_SP.png").get_rect()
    title.top = 50
    title.centerx = SCREEN_WIDTH/2
    SCREEN.blit(pygame.image.load("assets/title_SP.png"), title)
    ...
    while True:  # main loop for start screen interface
        ...
        if instructions_Screen == True:  # if current screen is Instructions Screen
            ...
            Text = ["Arrow keys to move, Esc to quit.   Eat all of the fruits to go to the next level.",
                    ...
                    "You will lose a life if you come into contact with a ghost. If you lose all lives, you will lose 10,000 points and start over.",
                    "The fruits' point values increase as the player goes to a higher level.",
                    ...
                    "4. Pac-Man is invulnerable to ghosts. (Super Pac-Man cannot eat ghosts however) 5. Double point value if fruit is eaten.",
...
def draw(mazeArray, gameDict, mazeScreen, reverseB, reverseP, reverseC, reverseI, superBlink, quickChange, directionP, blinkB, blinkP, blinkC, blinkI, X_SIZE, Y_SIZE):  # function to update screen (after player movement, etc.)
    ...
    if X_SIZE == 35 and Y_SIZE == 35:
        line = pygame.image.load("assets/door-35.png")
        lineV = pygame.image.load("assets/doorV-35.png")
    elif X_SIZE == 25 and Y_SIZE == 25:
        line = pygame.image.load("assets/door-25.png")
        lineV = pygame.image.load("assets/doorV-25.png")
    ...
    for x in range(len(mazeArray)):
        for y in range(len(mazeArray[x])):
            ...
            if (x, y) in gameDict["powerPellet"]:
                ...
                copyScreen.blit(pygame.image.load("assets/super_pellet.png"), spaceRect)
                ...
                if superBlink == True:
                    ...
                    if directionP == "up" and quickChange == False:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanU-S-B.png")
                    elif directionP == "up" and quickChange == True:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanU_C-S-B.png")
                    elif directionP == "right" and quickChange == False:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanR-S-B.png")
                    elif directionP == "right" and quickChange == True:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanR_C-S-B.png")
                    elif directionP == "left" and quickChange == False:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanL-S-B.png")
                    elif directionP == "left" and quickChange == True:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanL_C-S-B.png")
                    elif directionP == "down" and quickChange == False:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanD-S-B.png")
                    elif directionP == "down" and quickChange == True:
                        if num == 1:
                            image = pygame.image.load("assets/pacmanD_C-S-B.png")
                ...
                if fruitNum == 1:
                    copyScreen.blit(pygame.image.load("assets/cherry.jpg"), spaceRect)
                elif fruitNum == 2:
                    copyScreen.blit(pygame.image.load("assets/strawberry.jpg"), spaceRect)
                elif fruitNum == 3:
                    copyScreen.blit(pygame.image.load("assets/apple.jpg"), spaceRect)
                elif fruitNum == 4:
                    copyScreen.blit(pygame.image.load("assets/orange.jpg"), spaceRect)
                elif fruitNum == 5:
                    copyScreen.blit(pygame.image.load("assets/melon.jpg"), spaceRect)
                elif fruitNum == 6:
                    copyScreen.blit(pygame.image.load("assets/galaxian_boss.jpg"), spaceRect)
                else:
                    copyScreen.blit(pygame.image.load("assets/bell.jpg"), spaceRect)
            elif (x, y) in gameDict["keys"]:
                copyScreen.blit(pygame.image.load("assets/key.jpg"), spaceRect)
...
def drawOriginal(mazeArray, gameDict, num, levels, X_SIZE, Y_SIZE):
    ...
    if X_SIZE == 35 and Y_SIZE == 35:
        ...
        line = pygame.image.load("assets/line-25.png")
    ...
    if X_SIZE == 35 and Y_SIZE == 35:
        ...
        wall = pygame.image.load("assets/wall-25.png")
        blank = pygame.image.load("assets/wall_blank-25.png")
    for x in range(len(mazeArray)):
        for y in range(len(mazeArray[x])):
